  ...
  context('avai v2', async () => {
    ...
    it("Doesn't assign role to other uses", async () => {
      ...
      accounts.slice(1).forEach(async (account) => {
        expect(
          await avai.hasRole(DEFAULT_ADMIN_ROLE, account.address)
        ).to.equal(false);
      });

      accounts.forEach(async (account) => {
        expect(await avai.hasRole(MINTER_ROLE, account.address)).to.equal(
          false
        );
        expect(await avai.hasRole(BURNER_ROLE, account.address)).to.equal(
          false
        );
      });
    });
    ...
    it('gives minter and burner role to vault', async () => {
      ...
      expect(
        await avai.hasRole(await avai.MINTER_ROLE(), vaultAddress)
      ).to.equal(true);
      expect(
        await avai.hasRole(await avai.BURNER_ROLE(), vaultAddress)
      ).to.equal(true);
    });
  });
