...
describe('ERC20 Vault Test', function () {
  ...
  context('Bank upgrade', async () => {
    ...
    beforeEach(async function () {
      ...
      bankv2 = await bankv2Fac.deploy();
      expect(bankv2.address).to.be.properAddress;

      await v2.upgradeToNewBank(bankv2.address);
    });
    ...
    it('should allow debt to be borrowed with minimum debt and all variables are correct', async () => {
      // Set up
      ...
      // Set minimum debt
      await v2.setMinimumDebt(0, ethers.utils.parseEther('2.0'));

      await expect(
        wVault.borrowToken(2, ethers.utils.parseEther('1.0'))
      ).to.be.revertedWith('Borrow needs to be larger than the minimum debt');
