...
describe('Avax Vault Test with Gateway', function () {
  ...
  context('Bank upgrade', async () => {
    ...
    it('should let user get paid after liquidation', async () => {
      ...
      await v2.setPriceSource(0, fakePrice.address);

      const mintVal = await wVault.checkCost(2);
      // Let the user have minter role
      await v2.grantRole(await avai.MINTER_ROLE(), accounts[1].address);
      await v2.connect(accounts[1]).mint(accounts[1].address, mintVal.add(10)); // 1000 AVAI
