...
export const Deposit: FC<DepositProps> = ({
  ...
  const { data: farmBalance, mutate: mutateFarmBalance } = useSWR(
    shouldFetch ? [`tokenBalance${name}Staking`, library, account, farm] : null,
    getTokenBalance()
  );
  const { data: tokenIsApproved, mutate: mutateTokenApproved } = useSWR(
    shouldFetch
      ? [
          `tokenApproved${name}Staking`,
          library,
          account,
          chainId,
          farm,
  ...
  const handleApprove = async () => {
    setApproving(true);
    await handleTransaction({
      transaction: approveTokenxORCA(library, chainId, farm, 1e10),
      messages: {
  ...
  return (
    <FormikProvider value={formik}>
      <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
        <Grid container>
          <Grid item xs={5}>
            <Typography variant="subtitle1">ORCA Balance:</Typography>
          </Grid>
