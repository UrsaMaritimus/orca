...
export function PngVaults(props: PagesVaultsProps) {
  ...
  // Update to PNG vaults when deployed
  useEffect(() => {
    if (library) {
      const avaxVault = AVAXVault__factory.connect(
        isDev ? contractAddresses.fuji.AVAXVault : '',
  ...
  if (vaults) {
    return (
      <Card>
        ...
        />
        <MainTable rows={vaults} collateralType={'PNG'} debtType={'AVAI'} />
      </Card>
    );
  }
  return <> </>;
