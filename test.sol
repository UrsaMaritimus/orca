  ...
  function _checkOnERC721Received(
    ...
    if (to.isContract()) {
      ...
      returns (bytes4 retval) {
        return
          retval == IERC721ReceiverUpgradeable(to).onERC721Received.selector;
...
// File apps/avai/src/contracts/interfaces/IBaseVault.sol
...
// File apps/avai/src/contracts/interfaces/IStablecoin.sol
...
// File apps/avai/src/contracts/BaseVault.sol
...
// File apps/avai/src/contracts/WAVAXGateway.sol
...
contract WAVAXGateway is Ownable {
  ...
  modifier onlyVaultOwner(uint256 vaultID, address vault) {
    require(BaseVault(vault).vaultExistence(vaultID), 'Vault does not exist');
    require(
      BaseVault(vault).ownerOf(vaultID) == msg.sender,
      'Vault is not owned by you'
    );
    _;
  }
    ...
    onlyVaultOwner(vaultID, vault)
  {
    ...
    BaseVault(vault).depositCollateral(vaultID, msg.value);
  }
  ...
  function withdrawAVAX(
    ...
    BaseVault(vault).withdrawCollateral(vaultID, amount);
